  ...
  try {
    if (!JIRA_EMAIL || !JIRA_API_TOKEN || !JIRA_SITE) {
      return NextResponse.json(
        { error: 'Jira credentials not configured. Set JIRA_SITE, JIRA_EMAIL and JIRA_API_TOKEN environment variables.' },
        { status: 500 }
      );
    }
    ...
    const jql = `project = PROD AND type = Correção AND status IN (Produção, "Aguardando Deploy") AND updated >= "${startDate}" AND updated <= "${endDate}" AND assignee IN (${accountIdsJql}) ORDER BY cf[10019] ASC`;
    ...
    do {
      const params = new URLSearchParams({
        ...
        fields: 'summary,assignee,status,timespent,created,updated',
      });
